...
class RASWP_CPT {
	...
	private static function raswp_register_book_cpt() {
		register_post_type('raswp_book', [
			'label' => __('کتاب‌ها', 'wp-learn-word'),
			'labels' => [
				'name' => __('کتاب‌ها', 'wp-learn-word'),
				'singular_name' => __('کتاب', 'wp-learn-word')
			],
			'public' => true,
			'show_in_menu' => 'raswp',
	...
	private static function raswp_register_word_cpt() {
		register_post_type('raswp_word', [
			'label' => __('واژه‌ها', 'wp-learn-word'),
			'labels' => [
				'name' => __('واژه‌ها', 'wp-learn-word'),
				'singular_name' => __('واژه', 'wp-learn-word')
			],
			'public' => true,
			'show_in_menu' => 'raswp',
	...
	public static function raswp_register_meta_boxes() {
		add_action('add_meta_boxes', function() {
			add_meta_box('raswp_word_meta', __('جزئیات واژه', 'wp-learn-word'), [__CLASS__, 'raswp_render_word_meta'], 'raswp_word', 'normal', 'default');
		});
	...
	public static function raswp_render_word_meta($post) {
		...
		$example2 = get_post_meta($post->ID, 'raswp_example_2', true);
		$book_id = get_post_meta($post->ID, 'raswp_book_id', true);
		$books = get_posts(['post_type' => 'raswp_book', 'numberposts' => -1, 'post_status' => 'publish']);
		?>
		<p>
			<label for="raswp_translation"><strong><?php echo esc_html(__('ترجمه', 'wp-learn-word')); ?></strong></label><br/>
			<input type="text" name="raswp_translation" id="raswp_translation" value="<?php echo esc_attr($translation); ?>" class="widefat" />
		</p>
		<p>
			<label for="raswp_example"><strong><?php echo esc_html(__('جمله نمونه ۱', 'wp-learn-word')); ?></strong></label><br/>
			<textarea name="raswp_example" id="raswp_example" class="widefat" rows="3"><?php echo esc_textarea($example); ?></textarea>
		</p>
		<p>
			<label for="raswp_example_2"><strong><?php echo esc_html(__('جمله نمونه ۲', 'wp-learn-word')); ?></strong></label><br/>
			<textarea name="raswp_example_2" id="raswp_example_2" class="widefat" rows="3"><?php echo esc_textarea($example2); ?></textarea>
		</p>
		<p>
			<label for="raswp_book_id"><strong><?php echo esc_html(__('کتاب', 'wp-learn-word')); ?></strong></label><br/>
	...
	public static function raswp_save_word_meta($post_id) {
		...
		$example2 = isset($_POST['raswp_example_2']) ? sanitize_textarea_field($_POST['raswp_example_2']) : '';
		...
		update_post_meta($post_id, 'raswp_example_2', $example2);
		if ($book_id) {
